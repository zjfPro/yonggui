...
<script type="text/javascript">
	 function deleteChecked() {
           ...
           if (window.confirm("确认删除所选中的数据？")) {
        		$.ajax({
    				url : "<%=basePath%>control/accident/delete/deleteAccident",
    				data : "id=" + arrayValue,
...
function addChecked(){
	...
	content: 'url:<%=basePath%>control/accident/goto/addAccidentPage',	 //内文是页面，引入页面地址
...
function updateType(id){
 $.dialog({
	title: '修改车辆事故',
	content: 'url:<%=basePath%>control/accident/goto/updateAccidentPage?ccId='+id,	
	cancelVal: '关闭',				
	cancel: true, 
	width: '1000px',
	height: '580px',
...
<body>
	...
	<div class="content">
		<div class="content_box">
			<div class="current">
				<a>个人桌面</a> > <span></span> <a href="control/accident/select/accidentList">事故信息</a> > <span></span>事故列表
			</div>
			<div class="infolist">
						...
						<tbody>
							...
							<c:forEach items="${requestScope.pageUtil.datas}" var="data"
								...
								<tr class="hover_bg">
									...
									<td><fmt:formatDate value="${data.trafficAccidentTime}" pattern="yyyy-MM-dd"/></td>
									<td>${data.trafficAccidentAddress}</td>
									<td>${data.trafficAccidentInfo}</td>
									<td>${data.peccancyDescribe}</td>
									<td>${data.responsibility}</td>
									<td>${data.compensateMoney}</td>
