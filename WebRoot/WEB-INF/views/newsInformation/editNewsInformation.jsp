			...
			$("#addBtn").click(function() {
				if (!validate.form()) {
					...
					$.ajaxFileUpload({
						...
						url: "<%=basePath%>control/news/update/updateNewsInformation",
						secureuri: false, //是否需要安全协议，一般设置为false
...
<body>
	...
	<div class="content">
		<div class="content_box">
			...
			<div class="infolist">
				<form name="form" id="form" method="post" action="<%=basePath%>control/news/update/updateNewsInformation" enctype="multipart/form-data">
					<table class="left_table">
						<tr class="line_g">
							...
							<td>
								...
								<div id="display_img" style="width:100px;border:1px solid #000;overflow:hidden;">
									<c:if test="${requestScope.notice.picture == null }">
										<img id="imghead" style="width: 100px; height: 100px;"
											src="<%=request.getContextPath()%>/images/up_defaul.png" />
									</c:if>
									<c:if test="${requestScope.notice.picture != null }">
										<c:forEach items="${requestScope.notice.picture.split(',')}" var="img">
						...
						<tr class="line_g">
							<td width="10%">新闻标题：</td>
							<td>
								<input type="text" name="id" id="id" style="display:none" value="${requestScope.notice.id }"/>
								<input type="text" class="text width300" name="title" id="title" value="${requestScope.notice.title }"/>
							</td>
						</tr>
						<tr class="line_g">
							<td width="10%">新闻简介：</td>
							<td>
								<input type="text" class="text width300" name="noticeContentBrief" id="noticeContentBrief" value="${requestScope.notice.noticeContentBrief }"/>
							</td>
						</tr>
						<tr class="line_g">
							<td width="10%">新闻类型：</td>
							<td>
								<select name="type" id="type" style="width: 300px;">
									<option value="0" <c:if test="${requestScope.notice.type==0 }">selected="selected"</c:if>>公司新闻</option>
									<option value="1" <c:if test="${requestScope.notice.type==1 }">selected="selected"</c:if>>行业资讯</option>
							    </select>
							</td>
						</tr>
						<tr class="line_g">
							...
							<td>
								<select name="status" id="status" style="width: 300px;">
									<option value="0" <c:if test="${requestScope.notice.status==0 }">selected="selected"</c:if>>是</option>
									<option value="1"<c:if test="${requestScope.notice.status==1 }">selected="selected"</c:if>>否</option>
							    </select>
							</td>
						</tr>
						<tr class="line_g">
							<td width="10%">内容描述：</td>
							<td>
								<textarea id="editor" name="noticeContent" style="width:800px;height:200px;">${requestScope.notice.noticeContent }</textarea>
							</td>
						</tr>
