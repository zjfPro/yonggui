...
<script type="text/javascript">
	 function deleteChecked() {
           ...
           if (window.confirm("确认删除所选中的数据？")) {
        		$.ajax({
    				url : "<%=basePath%>control/news/delete/deleteNewsInformation",
					data : "ids=" + arrayValue,
					success : function(data) {
						if (data == "1") {
							window.location.href = "control/news/select/newsInformationList";
						} else {
							alert(data);
						}
					}
	...
	function edit(id){
		$.dialog({
			title: '编辑新闻资讯',
			content: 'url:<%=basePath%>control/news/goto/editNewsInformationView?id='+id,	
...
/* 	function checkDetail() {
		...
		if (selcount != 0) {
			if (selcount == 1) {
				var ccId = strs[0];
				window.location.href = "control/news/goto/editNewsInformationView?id="
						+ ccId;
			} else {
...
<body>
	...
	<div class="content">
		<div class="content_box">
			<div class="current">
				<a>个人桌面</a> > <span></span> <a href="javascript:changeRows();">新闻资讯</a>
			</div>
			<div class="infolist">
						...
						<thead>
							<tr>
								<td colspan="25">
									<div class="clear">
										<ul class="zsgc fleft">
											<!-- <li><a href="control/news/goto/addNewsInformation">新增</a></li> -->
						...
						<tbody>
							...
							<c:forEach items="${requestScope.pageUtil.datas}" var="data"
								...
								<tr class="hover_bg">
									...
									<td>
										<c:if test="${data.type==0}">公司新闻</c:if>
										<c:if test="${data.type==1}">行业资讯</c:if>
									</td>
									<td>
...
$("#addBtn").dialog({
		...
		content: "url:<%=basePath%>control/news/goto/addNewsInformation",	 //内文是页面，引入页面地址
