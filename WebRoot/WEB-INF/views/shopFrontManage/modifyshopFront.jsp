...
<body>
	...
	<div class="content">
		<div class="content_box">
			<div class="current">
				<a>个人桌面</a><span> > </span> <a
					href="shopFront/select/shopFrontList">门店管理</a><span>></span>编辑门店
			</div>
			<div class="infolist">
				<form name="form" id="form" method="post"
					action="shopFront/update/modifyshopFront"
					...
					<table class="left_table">
							...
							<td>
								...
								<div id="display_img"
									...
									<img id="imghead" style="width: 100px;height: 100px;"
										src="${requestScope.data.logo}" />
								</div> <a onclick="cleaninput('logo')">清空</a></td>
						</tr>
						<tr class="line_g">
							<td width="10%">门店名称：</td>
							<td><input type="text" class="text width300" name="name"
								id="name" value="${requestScope.data.name}" /><a
								onclick="cleaninput('name')">清空</a></td>
						</tr>

						<tr class="line_g">
							<td width="10%">门店地址：</td>
							<td><input type="text" class="text width300" name="address"
								id="address" value="${requestScope.data.address}" /><a
								onclick="cleaninput('address')">清空</a></td>
						</tr>
						<tr class="line_g">
							<td width="10%">门店地址经度：</td>
							<td><input type="text" class="text width300"
								name="longitude" id="longitude"
								value="${requestScope.data.longitude}" /><a
								onclick="cleaninput('longitude')">清空</a></td>
						</tr>
						<tr class="line_g">
							<td width="10%">门店地址纬度：</td>
							<td><input type="text" class="text width300" name="latitude"
								id="latitude" value="${requestScope.data.latitude}" /><a
								onclick="cleaninput('latitude')">清空</a></td>
						</tr>
						<tr class="line_g">
							<td width="10%">门店介绍：</td>
							<td colspan="2"><div class="table_area">
									<textarea id="editor_id" name="introduce"
										style="width:1000px;height:350px;">${requestScope.data.introduce}</textarea>
								</div> <a onclick="cleaninput('editor_id')">清空</a></td>
						</tr>
						...
						<c:if test="${sessionScope.admin!=null }">
							<tr class="line_g">
								<td width="10%">店长设置：</td>
								<td><input type="text" class="text width300"
									value="${requestScope.data.manageStaff.name}"
									name="manageStaff_name" id="manageStaff_name"
									onblur="searchStaff()"
									onfocus="if (this.value=='请输入员工编号' || this.value=='没有结果'){this.value=''}" />
									<input type="hidden" name="manageStaffId" id="manageStaff_id"
									value="${requestScope.data.manageStaff.id}" /> <a
									onclick="cleaninput('manageStaff_name')">清空</a></td>
							</tr>
						</c:if>
