...
<script type="text/javascript">
	 function deleteChecked() {
           ...
           if (window.confirm("确认删除所选中的数据？")) {
        		$.ajax({
    				url : "<%=basePath%>control/setting/delete/deleteSetting",
         ...
         function checkDetail() {
            ...
            if(selcount!=0){
            	if(selcount==1){
            		var ccId = strs[0];
            	    window.location.href="control/setting/goto/updateSettingPage?ccId="+ccId; 
...
function addChecked(){
	...
	content: 'url:<%=basePath%>control/setting/goto/addSettingPage',	 //内文是页面，引入页面地址
...
function updateType(id){
 $.dialog({
	title: '修改库存',
	content: 'url:<%=basePath%>control/setting/goto/updateSettingPage?ccId='+id,	
	cancelVal: '关闭',				
	cancel: true, 
	width: '580px',
	height: '300px',
...
<body>
	...
	<div class="content">
		<div class="content_box">
			<div class="current">
				<a>个人桌面</a> > <span></span> <a href="control/setting/select/settingList">车辆库存</a> > <span></span>库存列表
			</div>
			<div class="infolist">
								...
								<td colspan="16">
									<div class="clear">
										...
										<div class="searchbox fright">
											<input class="sea_text" type="text" name="text" value="${requestScope.text}" placeholder="门店"/><input
						...
						<tbody>
							...
							<c:forEach items="${requestScope.pageUtil.datas}" var="data"
								...
								<tr class="hover_bg">
									...
									<td>${data.carStockMinAlert}</td>
									<td>${data.carStockMaxAlert}</td>
