...
<script type="text/javascript">
	 function deleteChecked() {
           ...
           if (window.confirm("确认删除所选中的数据？")) {
        		$.ajax({
    				url : "<%=basePath%>control/avmc/delete/deleteAnnualVerification",
					data : "ids=" + arrayValue,
					success : function(data) {
						if (data == "1") {
							window.location.href = "control/avmc/select/annualVerificationList";
						} else {
							alert(data);
						}
					}
	...
	function edit(id){
		$.dialog({
			title: '编辑年审',
			content: 'url:<%=basePath%>control/avmc/goto/editAnnualVerificationView?id='+id,	
...
<body>
	...
	<div class="content">
		<div class="content_box">
			<div class="current">
				<a>个人桌面</a> > <span></span> <a href="javascript:changeRows();">年审管理</a>
			</div>
			<div class="infolist">
						...
						<thead>
							<tr>
								<td colspan="25">
									<div class="clear">
										<ul class="zsgc fleft">
											<!-- <li><a href="control/avmc/goto/addAnnualVerification">新增</a></li> -->
						...
						<tbody>
							...
							<c:forEach items="${requestScope.pageUtil.datas}" var="data"
								...
								<tr class="hover_bg">
									...
									<%-- ${data.parent.info }(门店：${data.fronts.name },车牌：${data.plateNumber} --%>
									<td>${data.carInfo.carType.carBrand.info}(门店：${data.carInfo.shopFront.name},车牌：${data.carInfo.plateNumber} )</td>
									<td>${data.opinion}</td>
									<td>
										<c:if test="${data.ispass==1}">审核通过</c:if>
										<c:if test="${data.ispass==0}">未通过</c:if>
									</td>
									<td>
...
$("#addBtn").dialog({
		...
		content: "url:<%=basePath%>control/avmc/goto/addAnnualVerification",	 //内文是页面，引入页面地址
