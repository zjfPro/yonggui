					...
					<table class="nav_tab">
						<tbody>
							<tr>
								<th>门店:</th>
								<td>
									<input type="text" style="display:none" name="id" id="id" value="${requestScope.dept.id}" />
								    <select name="shopFrontId" id="shopFrontId" class="nt-select" style="width:300px" onchange="shopFrontChange(this.value)">
										...
										<c:forEach items="${requestScope.shopFrontList }" var="data">
											<option value="${data.id }" <c:if test="${data.id==requestScope.dept.shopFrontId }">selected="selected"</c:if>>${data.name }</option>
										</c:forEach>
								    </select>
								</td>
							</tr>
							...
							<tr>
								<th>部门名称：</th>
								<td colspan="7">
									<input type="text" class="nt-text" name="name" id="name" value="${requestScope.dept.name}" style="width:300px"/>
								</td>
							</tr>
						</tbody>
