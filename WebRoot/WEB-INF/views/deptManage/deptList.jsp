...
<script type="text/javascript">
	 function deleteChecked() {
           ...
           if (window.confirm("确认删除所选中的数据？")) {
        		$.ajax({
    				url : "<%=basePath%>control/dept/delete/deleteDept",
					data : "ids=" + arrayValue,
					success : function(data) {
						if (data == "1") {
							window.location.href = "control/dept/select/deptList";
						} else if(data == "0"){
							alert("该部门下还有员工，不能删除。");
							window.location.href = "control/dept/select/deptList";
						}
	...
	function edit(id){
		$.dialog({
			title: '编辑部门',
			content: 'url:<%=basePath%>control/dept/goto/editDeptView?id='+id,	
...
<body>
	...
	<div class="content">
		<div class="content_box">
			<div class="current">
				<a>个人桌面</a> > <span></span> <a href="javascript:changeRows();">部门管理</a>
			</div>
			<div class="infolist">
						...
						<tbody>
							...
							<c:forEach items="${requestScope.pageUtil.datas}" var="data"
								...
								<tr class="hover_bg">
									...
									<td>${data.name}</td>
									<td>${data.staffManageRoleContainer.name}</td>
									<td>
...
$("#addBtn").dialog({
		...
		content: "url:<%=basePath%>control/dept/goto/addDept",	 //内文是页面，引入页面地址
